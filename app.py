...
def get_volume(pair, general_price, exchange):
    ...
    try:
        ...
        currency_volume = volume * general_price
        
        if currency_volume > volume_needed:
...
def main_function():
    ...
    for base_currency in base_currencies[1:]:
        ...
        for opportunity_pair in tqdm(opportunity_pairs):
            ...
            confidence = confidence_check(opportunity_pairs[0], price_volatility)
            
            if confidence != False:
                    
                prices_message = 'Buy '+ opportunity_pairs[i][1] +' on: '+ opportunity_pairs[i][2][0] +' ('+ str(opportunity_pairs[i][2][1]) +')\nSend to: '+ opportunity_pairs[i][3][0]+' ('+ str(opportunity_pairs[i][2][1]) +')\nOpportunity: '+ str(opportunity_pairs[i][0])+  '\nConfidence: ' + str(confidence)
